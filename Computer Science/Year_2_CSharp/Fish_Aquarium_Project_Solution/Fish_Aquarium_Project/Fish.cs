...
namespace Fish_Aquarium_Project
{
    class Fish : PictureBox
    {
        ...
        private int x, y;//help with positioning the fish
        ...
        public void Swim(Timer timer1, ref Food[] FoodArray, ref int FoodArrayLength)
        {
            ...
            if (dead)
            {
                // If they are dead they go up
                Invoke((MethodInvoker)delegate
                {
                    Location = new Point(Location.X - randy.Next(-1, 2), Location.Y - 2);
                });
            }
            else if (FoodArray.Length > 0 && !FoodArray.Contains(followedFood))
            {
