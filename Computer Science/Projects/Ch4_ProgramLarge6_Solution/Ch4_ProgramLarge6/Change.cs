...
namespace Ch4_ProgramLarge6
{
    class Change : Colors
    {
        ...
        static void Main(string[] args)
        {
            for (int i = 0; i < 100; i++)                                        //Error looping
            {
                try
                {
                    ...
                    TotalChangeCalcForDollars(totalChange); //Calc and display of dollars
                    TotalChangeCalcForCoins(totalChange); //Calc and display of coins
        ...
        public static void TotalChangeCalcForCoins(decimal x)
        {
            ...
            int amountOfQuarters = (int)(((((x % WORTH_OF_TWENTY_DOLLARS) % WORTH_OF_TEN_DOLLARS)
                    % WORTH_OF_FIVE_DOLLARS) % WORTH_OF_A_DOLLAR) / WORTH_OF_A_QUARTER),

                amountOfDimes = (int)((((((x % WORTH_OF_TWENTY_DOLLARS) % WORTH_OF_TEN_DOLLARS)
                    % WORTH_OF_FIVE_DOLLARS) % WORTH_OF_A_DOLLAR) % WORTH_OF_A_QUARTER) 
                    / WORTH_OF_A_DIME),

                amountOfNickels = (int)(((((((x % WORTH_OF_TWENTY_DOLLARS) % WORTH_OF_TEN_DOLLARS)
                    % WORTH_OF_FIVE_DOLLARS) % WORTH_OF_A_DOLLAR) % WORTH_OF_A_QUARTER) 
                    % WORTH_OF_A_DIME) / WORTH_OF_A_NICKEL),

                amountOfPennies = (int)Math.Floor(((((((((x % WORTH_OF_TWENTY_DOLLARS) 
                    % WORTH_OF_TEN_DOLLARS) % WORTH_OF_FIVE_DOLLARS) % WORTH_OF_A_DOLLAR)
                    % WORTH_OF_A_QUARTER) % WORTH_OF_A_DIME) % WORTH_OF_A_NICKEL) 
                    / WORTH_OF_A_PENNY));
            #endregion
