...
module.exports = {
	...
	usage: 'help <command>',
	description: 'View a list of commands or get information about a specific command from Stan\'s Secretary! Sorry.... he doesnt have time for you...',
	...
	execute: (client, message, args) => {
		if (args.length > 0) {
			...
			if (command.length > 0) {
				...
				.setFooter(config.strings.stan)
				.setTimestamp();
				message.channel.send({ embed }).then(message.delete(180000));
			} else {
				const embed = new RichEmbed()
			...
			.setFooter(config.strings.stan)
			.setTimestamp();
			message.channel.send({ embed: embed2 }).then(message.delete(180000));
		}}
	};
