...
module.exports = {
	...
	execute: (client, message, args) => {
		if(message.author.id = '250815960250974209' | '235543144597291008') {
			if (args.length > 0) {
				...
				if (filtered.length > 0) {
					...
					try {
						...
						message.channel.send('An error occured while assigning command to self. Restoring in-memory command to before reload.\n```js\n' + util.inspect(error) + '```');
